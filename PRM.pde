...
ArrayList<Integer> visited = new ArrayList(); //A list which store if a given node has been visited
boolean[] visitedBFS = new boolean[maxNumNodes];
...
int[] closestNode(Vec2[]centers, float[] radii, int numObstacles, Vec2 startPos, Vec2 goalPos, Vec2[] nodePos, int numNodes){
  int[] closestID = new int[2];
  float minDist = 999999;
  //float closestGoal = 999999;
  
  for (int i = 0; i < numNodes; i++){
    float distStart = nodePos[i].distanceTo(startPos);
    Vec2 startDir = nodePos[i].minus(startPos).normalized();
    hitInfo startCheck = rayCircleListIntersect(centers, radii, numObstacles, startPos, startDir, distStart);
    for (int j = 0; j < numNodes; j++){
      if(i != j){
        float distGoal = nodePos[j].distanceTo(goalPos);
        float distNodes = nodePos[i].distanceTo(nodePos[j]);
        Vec2 goalDir = nodePos[j].minus(goalPos).normalized();
        hitInfo goalCheck = rayCircleListIntersect(centers, radii, numObstacles, goalPos, goalDir, distGoal);
        //float iGoal = goalPos.distanceTo(nodePos[i]);
        float dist = distStart + distGoal + distNodes;
        if(!startCheck.hit && !goalCheck.hit){
          if(dist < minDist){
            closestID[0] = i;
            closestID[1] = j;
            //closestGoal = iGoal;
  ...
  /*
  int closestGoalID = -1;
  float minGoalDist = 999999;
  for (int i = 0; i < numNodes; i++){
    float dist = nodePos[i].distanceTo(goalPos);
    if (dist < minGoalDist){
      closestGoalID = i;
      minGoalDist = dist;
    }
  }
  closestID[1] = closestGoalID;
  */
  return closestID;
}
...
ArrayList<Integer> planPath(Vec2 startPos, Vec2 goalPos, Vec2[] centers, float[] radii, int numObstacles, Vec2[] nodePos, int numNodes){
  ...
  int[] startGoalIDs = closestNode(centers, radii, numObstacles, startPos, goalPos, nodePos, numNodes);
  
  int startID = closestNode(startPos, nodePos, numNodes);
  //int startID = startGoalIDs[0];
  int goalID = closestNode(goalPos, nodePos, numNodes);
  //int goalID = startGoalIDs[1];
...
//Below I've attached two of the links I used to help me understand and write this algorithm.
...
ArrayList<Integer> runUCS(Vec2[] nodePos, int numNodes, int startID, int goalID){
  ...
  for (int i = 0; i < numNodes; i++) { //Clear parent pointers
  ...
  while(queue.size() > 0){
   ...
   for(int i = 0; i < neighbors[current].size(); i++){
     int child = neighbors[current].get(i);
     if(!explored.contains(child) && !queue.contains(child)){
       ...
       if(pathCost[child] < pathCost[current]){
         parent[child] = current;
         explored.add(child);
         queue.add(child);
       }
       ...
       float cost = nodePos[child].distanceTo(nodePos[current]);
       pathCost[child] = pathCost[current] + cost;
     }
   }
  }
  if (queue.size() == 1){
...
ArrayList<Integer> runBFS(Vec2[] nodePos, int numNodes, int startID, int goalID){
  ...
  while (fringe.size() > 0){
    ...
    for (int i = 0; i < neighbors[currentNode].size(); i++){
      ...
      if (!visitedBFS[neighborNode]){
        visitedBFS[neighborNode] = true;
